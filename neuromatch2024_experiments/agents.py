...
class LSTMDQNAgent(nn.Module):
    ...
    def learn(self):
        ...
        hidden_tensor = (
            torch.stack([h[0].clone().detach().requires_grad_(True).squeeze(
                0).squeeze(0) for h in batch.hidden]).unsqueeze(0).to(self.device),
            torch.stack([h[1].clone().detach().requires_grad_(True).squeeze(
                0).squeeze(0) for h in batch.hidden]).unsqueeze(0).to(self.device)
        )
        ...
        next_hidden_tensor = (
            torch.stack([h[0].clone().detach().requires_grad_(True).squeeze(0).squeeze(
                0) for h in batch.next_hidden]).unsqueeze(0).to(self.device),
            torch.stack([h[1].clone().detach().requires_grad_(True).squeeze(
                0).squeeze(0) for h in batch.next_hidden]).unsqueeze(0).to(self.device)
        )
        with torch.no_grad():
            next_q_values, _ = self.q_network(
                next_states_tensor, next_hidden_tensor)
